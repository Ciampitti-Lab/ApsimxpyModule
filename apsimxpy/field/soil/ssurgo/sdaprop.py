...
def numprops(show = False):
    ...
    numdic = {'0.1 bar H2O - Rep Value': 'wtenthbar_r',
...
def getprop(df, column=str, prop=None, method=None,  top=None, bottom=None, minmax=None, prnt=None, meta=None):
    ...
    try:
        ...
        if method == "dom_comp_cat":
            ...
            SELECT areasymbol, musym, muname, mu.mukey  AS mukey,ch.hzname, ch.hzdept_r, ch.hzdepb_r, """ + aProp + """ AS """ + aProp +"""
            FROM legend  AS l
        ...
        if method == "minmax":
            q = f"""-- min or max horizon with depths
            SELECT 
                l.areasymbol, 
                mu.musym, 
                mu.muname, 
                ...
                ch.{aProp} AS {aProp},
                CASE WHEN ch.{aProp} = (
                    SELECT {minmax}(ch2.{aProp})
                    FROM chorizon AS ch2
                    WHERE ch2.cokey = c.cokey
                ) THEN 1 ELSE 0 END AS is_minmax
            FROM legend AS l
            ...
            INNER JOIN component AS c 
                ...
                AND c.cokey = (
                    ...
                    WHERE c1.mukey = mu.mukey
                    ORDER BY c1.comppct_r DESC, c1.cokey
                )
            INNER JOIN chorizon AS ch 
                ON ch.cokey = c.cokey
            ORDER BY l.areasymbol, mu.musym, mu.muname, mu.mukey, ch.hzdept_r
