...
gtd2=gtd2[['year','id','rtotn_kgha','ry15_mtha','county','year']]
...
def fitting_curves(group):
    quad_results = fit_quadratic_plateau(group, 'ry15_mtha', "rtotn_kgha")
    lin_results = fit_linear_plateau(group, 'ry15_mtha', "rtotn_kgha")
    

    return pd.Series({
        'quad_r2':   quad_results['r2'],
        'lin_r2':    lin_results['r2'],
        'quad_rmse': quad_results['rmse'],
        'lin_rmse':  lin_results['rmse'],
        'quad_b0':   quad_results['b0'],
        'lin_b0':    lin_results['b0'],
        'quad_b1':   quad_results['b1'],
        'lin_b1':    lin_results['b1'],
        'quad_b2':   quad_results.get('b2', np.nan) ,
        'quad_x_break': quad_results['x_break'],
        'lin_x_break': lin_results['x_break']
        
    })
    
fit_results = gtd2.groupby('id',as_index=False).apply(fitting_curves, include_groups=True)
...
x_break=np.where(fit_results['best_model']=='quad_model',fit_results['quad_x_break'],fit_results['lin_x_break'])

max_rate=gtd2.groupby('id',as_index=False)['rtotn_kgha'].max()

# aonr=gtd2.groupby('id',as_index=False)['ry15_mtha'].max[]
...
gtd2_curves=pd.DataFrame({
    ...
    'c':c,
    'x_break':x_break,
    'max_rate' : max_rate['rtotn_kgha']
    }
)

# Nitrogen dosis
nitro = np.random.uniform(0, 300, size=200)
...
gtd2_curves['aonr']=np.minimum(gtd2_curves['x_break'],gtd2_curves['max_rate'])

gtd2_curves=gtd2_curves[['id','effective_r','yield_ton','years','county','region','aonr']]
gtd2_curves['dbase'] = 'GTD2'
gtd2_curves.columns=['id','nitro_kg_ha','yield_ton_ha','year','county','region','aonr','dbase']
gtd2_curves.head(564)

# Variables to have at the end of the workflow yield_ton_ha	nitro_kg_ha	county	region	year	id	aonr	dbase
